...
#include "ArenaTypes.h"  // ArenaPoint / ArenaHeading / RFIDLocation / Test8ReviveTarget
...
enum class MissionPhase {
  ...
  Tunnel,
  Arena,
  ArenaToEntryGate,      // line-follow inside arena toward Tunnel A surface door
  TunnelReturn,          // inbound airlock: through Tunnel A back into the base
  BaseReturnToParking    // line-follow inside base to the robot collection area
};
...
// Inbound airlock sub-states (Tunnel A). Mirror of TunnelState
// but the doors open in response to a WiFi request to the
// server instead of being detected passively.
enum class TunnelReturnState {
  ApproachGate,          // close in on the surface-side door (forward US triggers)
  RequestOuterDoor,      // send WiFi "open Tunnel A" request (stubbed)
  WaitOuterDoor,         // poll forward US until the door has opened
  DrivingIntoAirlock,    // crawl forward into the airlock past the door frame
  WallFollow,            // follow ramp wall down to the base-side door
  RequestInnerDoor,      // send WiFi "open inner door" request (stubbed)
  WaitInnerDoor,         // poll forward US until the inner door has opened
  PassingInnerDoor,      // drive through into the base
  Done                   // hand off to BaseReturnToParking
};
...
TunnelReturnState tunnel_return_state = TunnelReturnState::ApproachGate;

// Arena pathfinder goal. Higher-level mission code (find next fertile
// cell, head for parking, etc.) sets these; the Arena phase consumes
// them in option (b). When arena_have_goal == false, the swerve runs.
ArenaPoint arena_goal = {-1, -1};
bool arena_have_goal = false;
...
// =====================================================
// Tunnel return (inbound airlock via Tunnel A)
// Mirror of the tunnel-entry block above. Reuses
// read_ultrasonic_mm(), forward_distance_mm(),
// side_distance_mm(), wall_follow_step(), drive_into_tunnel(),
// BASE_EXIT_MIN_DISTANCE_COUNTS, BASE_EXIT_DRIVE_TIMEOUT_MS,
// SIDE_WALL_DETECTED_MM.
//
// Entry precondition (until a coordinate / nav layer exists):
// robot is roughly one grid unit in front of Tunnel A's
// surface-side door, on the line, facing the door.
// =====================================================

constexpr float RETURN_GATE_STOP_MM         = 100.0f;
constexpr float RETURN_OUTER_DOOR_OPEN_MM   = 250.0f;
constexpr float RETURN_INNER_DOOR_CLOSED_MM = 50.0f;
constexpr float RETURN_INNER_DOOR_OPEN_MM   = 250.0f;
constexpr float RETURN_DONE_MM              = 500.0f;

constexpr int16_t RETURN_APPROACH_SPEED = 220;

unsigned long tunnel_return_state_start_ms = 0;
long          tunnel_return_drive_in_start_counts = 0;

// Real door-open requests via MQTT to the server. The tag_id
// passed is whatever the last fertility/localisation reply
// referenced — that's the UID of the doorway tag the robot
// just rolled over while pathfinding into the gate cell.
bool request_open_outer_door()
{
  const bool ok = messagesSendOpenAirlock('A', message_last_fertility_tag);
  Serial.print("messagesSendOpenAirlock(A, outer) ok=");
  Serial.println(ok ? 1 : 0);
  return true;  // advance regardless; door open is observed via ultrasonic
}

bool request_open_inner_door()
{
  const bool ok = messagesSendOpenAirlock('A', message_last_fertility_tag);
  Serial.print("messagesSendOpenAirlock(A, inner) ok=");
  Serial.println(ok ? 1 : 0);
  return true;
}

void set_tunnel_return_state(TunnelReturnState next)
{
  if (tunnel_return_state == next) {
    return;
  }
  tunnel_return_state = next;
  tunnel_return_state_start_ms = millis();

  if (next == TunnelReturnState::DrivingIntoAirlock) {
    tunnel_return_drive_in_start_counts = getLeftEncoder();
  }
}

void setup_tunnel_return()
{
  tunnel_return_state = TunnelReturnState::ApproachGate;
  tunnel_return_state_start_ms = millis();
}

void run_tunnel_return_step()
{
  const float fwd  = forward_distance_mm();
  const float side = side_distance_mm();

  switch (tunnel_return_state) {

    case TunnelReturnState::ApproachGate:
      // Pathfind to the Tunnel A doorway cell (9,7). This is blocking
      // and will spend the whole arena traversal inside this one call,
      // driving one cell at a time with RFID localisation + obstacle
      // re-planning. On success we are sitting on the gate tag and
      // facing the door.
      stopMotors();
      if (arenaGoToExit()) {
        Serial.println("Reached Tunnel A doorway cell via pathfinder.");
        mission_phase = MissionPhase::TunnelReturn;
        set_tunnel_return_state(TunnelReturnState::RequestOuterDoor);
      } else {
        Serial.println("arenaGoToExit failed; will retry next tick.");
      }
      break;

    case TunnelReturnState::RequestOuterDoor:
      stopMotors();
      if (request_open_outer_door()) {
        set_tunnel_return_state(TunnelReturnState::WaitOuterDoor);
      }
      break;

    case TunnelReturnState::WaitOuterDoor:
      stopMotors();
      if (fwd > RETURN_OUTER_DOOR_OPEN_MM) {
        Serial.println("Tunnel A surface door opened.");
        set_tunnel_return_state(TunnelReturnState::DrivingIntoAirlock);
      }
      break;

    case TunnelReturnState::DrivingIntoAirlock:
      drive_into_tunnel();
      {
        const long traveled = getLeftEncoder() - tunnel_return_drive_in_start_counts;
        if (traveled < BASE_EXIT_MIN_DISTANCE_COUNTS) {
          break;
        }
      }
      if (side < SIDE_WALL_DETECTED_MM) {
        Serial.println("Side wall found - wall following (return).");
        set_tunnel_return_state(TunnelReturnState::WallFollow);
      } else if (millis() - tunnel_return_state_start_ms > BASE_EXIT_DRIVE_TIMEOUT_MS) {
        Serial.println("No wall after timeout - wall following anyway (return).");
        set_tunnel_return_state(TunnelReturnState::WallFollow);
      }
      break;

    case TunnelReturnState::WallFollow:
      // Assumes LEFT wall, matching outbound. If Tunnel A's wall
      // sits on the right on the return trip, mirror this call.
      wall_follow_step();
      if (fwd < RETURN_INNER_DOOR_CLOSED_MM) {
        Serial.println("Reached Tunnel A base-side door.");
        stopMotors();
        set_tunnel_return_state(TunnelReturnState::RequestInnerDoor);
      }
      break;

    case TunnelReturnState::RequestInnerDoor:
      stopMotors();
      if (request_open_inner_door()) {
        set_tunnel_return_state(TunnelReturnState::WaitInnerDoor);
      }
      break;

    case TunnelReturnState::WaitInnerDoor:
      stopMotors();
      if (fwd > RETURN_INNER_DOOR_OPEN_MM) {
        Serial.println("Tunnel A base-side door opened.");
        set_tunnel_return_state(TunnelReturnState::PassingInnerDoor);
      }
      break;

    case TunnelReturnState::PassingInnerDoor:
      wall_follow_step();
      if (fwd > RETURN_DONE_MM) {
        Serial.println("Through Tunnel A - entering base.");
        stopMotors();
        set_tunnel_return_state(TunnelReturnState::Done);
        mission_phase = MissionPhase::BaseReturnToParking;
      }
      break;

    case TunnelReturnState::Done:
      // Hand-off to BaseReturnToParking handler.
      stopMotors();
      break;
  }
}
...
void loop()
{
  serviceServoPulses();
  update_turn_angle();
  messagesLoop();  // service WiFi/MQTT every tick; safe if WiFi is down
  ...
  if (Serial.available() > 0) {
    ...
    if (command == '9') {
      // RFID + server test: scan one tag, send isFertile to the server,
      // print the raw reply, fertility, and x/y coordinates.
      runRFIDServerTest();
      return;
    }
    ...
    if (command == '4') {
      // Trigger return-to-base flow: pathfind to Tunnel A doorway
      // (9,7) then run the inbound airlock state machine.
      setup_tunnel_return();
      mission_phase = MissionPhase::ArenaToEntryGate;
      Serial.println("Return-to-base sequence selected.");
      return;
    }

    if (command == '7') {
      // Run the revival mission (Test 8).
      RunTest8();
      return;
    }
  }
  ...
  switch (mission_phase) {
    ...
    case MissionPhase::Arena:
      // Option (b): pathfinder primary, swerve as fallback.
      // arena_have_goal is set by higher-level mission code (e.g. "go to
      // the nearest fertile cell") — wiring for that doesn't exist yet,
      // so by default we still run the trial-tested swerve.
      if (arena_have_goal) {
        if (arenaPathfindTo(arena_goal)) {
          arena_have_goal = false;  // reached it; mission code will set the next goal
        } else {
          // Path planning or driving failed (no pose / no path / obstacle
          // blocked). Fall back to swerve for one tick so we keep moving
          // instead of locking up, then mission code can re-plan.
          if (now_ms - last_control_ms >= CONTROL_INTERVAL_MS) {
            last_control_ms = now_ms;
    ...
    case MissionPhase::ArenaToEntryGate:
    case MissionPhase::TunnelReturn:
      run_tunnel_return_step();
      break;

    case MissionPhase::BaseReturnToParking:
      // Placeholder until base-side parking line-follow exists.
      if (now_ms - last_control_ms >= CONTROL_INTERVAL_MS) {
        last_control_ms = now_ms;
        update_line_following();
      }
