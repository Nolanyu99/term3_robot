...
bool turnAngleOnly(float max_degrees, int8_t direction)
{
  ...
  Serial.print("turn_angle_only_start direction=");
  Serial.print(direction > 0 ? "left" : "right");
  Serial.print(" max_deg=");
  Serial.println(max_degrees, 1);
  ...
  while (fabsf(turn_angle_deg) < max_degrees) {
    ...
    if (checkStopInputsDuringTest()) {
      stopMotors();
      Serial.println("turn_angle_only=aborted");
      return false;
    }
    ...
    if (millis() - start_ms >= TURN_TIMEOUT_MS) {
      Serial.print("turn_angle_only_timeout angle=");
      Serial.println(turn_angle_deg, 1);
      stopMotors();
      return false;
    }
  ...
  Serial.print("turn_angle_only_done angle=");
  Serial.println(turn_angle_deg, 1);
