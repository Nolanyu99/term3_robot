...
bool driveStraightUntilRFIDAndPlant(uint8_t target_scans)
{
  uint8_t scans = 0;

  Serial.print("drive_straight_until_rfid target=");
  Serial.println(target_scans);

  while (scans < target_scans) {
    serviceServoPulses();
    update_turn_angle();

    if (checkStopInputsDuringTest()) {
      stopMotors();
      Serial.println("drive_straight_until_rfid=aborted");
      return false;
    }
    ...
    if (scanRFIDForTest()) {
      ...
      if (seeds > 0) {
        ...
        Serial.print("seed_planted remaining=");
        Serial.println(seeds);
      } else {
        Serial.println("seed_error=no_seeds_remaining");
      }
  ...
  Serial.println("drive_straight_until_rfid=done");
  return true;
}
...
{
  if (startup_cal_state != StartupCalState::Ready) {
    Serial.println("test4_error=not_calibrated");
    return;
  }

  if (!running) {
    Serial.println("test4_error=not_running");
    return;
  }

  if (!itg320x_ready) {
    Serial.println("test4_error=gyro_not_ready");
    stopMotors();
    return;
  }
  ...
  // Stage 1: drive forward until 2 RFID scans, planting each time.
  Serial.println("test4_stage=forward_until_2_scans");

  if (!driveStraightUntilRFIDAndPlant(2)) {
    Serial.println("test4=aborted_stage1");
    return;
  }

  // Turn right 90 degrees using IMU only.
  Serial.println("test4_stage=right_90");

  if (!turnAngleOnly(TURN_90_TARGET_DEG, -1)) {
    Serial.println("test4=aborted_right_turn");
    return;
  }

  if (checkStopInputsDuringTest()) {
    Serial.println("test4=aborted_after_right_turn");
    return;
  }

  // Stage 2: drive forward until 1 RFID scan, planting.
  Serial.println("test4_stage=forward_until_1_scan");

  if (!driveStraightUntilRFIDAndPlant(1)) {
    Serial.println("test4=aborted_stage2");
    return;
  }

  // Turn left 90 degrees using IMU only.
  Serial.println("test4_stage=left_90");

  if (!turnAngleOnly(TURN_90_TARGET_DEG, 1)) {
    Serial.println("test4=aborted_left_turn");
    return;
  }

  if (checkStopInputsDuringTest()) {
    Serial.println("test4=aborted_after_left_turn");
    return;
  }

  // Stage 3: drive forward until 2 more RFID scans, planting each time.
  Serial.println("test4_stage=forward_until_2_more_scans");

  if (!driveStraightUntilRFIDAndPlant(2)) {
    Serial.println("test4=aborted_stage3");
    return;
  }
