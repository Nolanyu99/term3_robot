...
constexpr unsigned long LINE_STATUS_INTERVAL_MS = 3000;
...
constexpr float TURN_90_TARGET_DEG = 95.0f; // Max turn before second guessing
constexpr float TURN_180_TARGET_DEG = 190.0f;
constexpr float TURN_90_IGNORE_LINE_UNTIL_DEG = 55.0f; // Minimum turn before reading scanner
constexpr float TURN_180_IGNORE_LINE_UNTIL_DEG = 120.0f;

constexpr uint16_t TURN_LINE_DETECT_THRESHOLD = 550;
constexpr uint16_t TURN_CENTER_DETECT_THRESHOLD = 500;
...
void loop()
{
  ...
  if (startup_cal_state == StartupCalState::Ready) {
    update_turn_angle(); // Update angle only after startup calibration
  }
  ...
  if (Serial.available() > 0) {
    ...
    if (command == '0') { // If 0 stop
    ...
    else if (command == '8') { // If 8 run rescue/revive test
      RunTest8();
      return;
    }

    else if (command == 'r' || command == 'R') { // Manually add revive target
      ...
      test8AddOrUpdateReviveTarget("manual", static_cast<int8_t>(x), static_cast<int8_t>(y));

      Serial.print("manual_revive_target_added x=");
      Serial.print(x);
      Serial.print(" y=");
      Serial.println(y);
  ...
  if (startup_cal_state != StartupCalState::Ready) {
    ...
    if (startup_cal_state == StartupCalState::CalibratingIMU) {
      ...
      startup_cal_state = StartupCalState::StartingWiFi;
      Serial.println("IMU calibration complete.");
      Serial.println("GREEN LED: starting WiFi/MQTT connection attempts.");
      return;
    }

    if (startup_cal_state == StartupCalState::StartingWiFi) {
      Green();

      setupMessages();   // Starts WiFi/MQTT retry logic
      messagesLoop();    // Immediately performs the first retry/service pass

      startup_cal_state = StartupCalState::Ready;
      calibration_done = true;

      Serial.println("All calibration complete. Robot ready.");
      Serial.println("WiFi/MQTT will continue retrying in the background if unavailable.");
      return;
    }
  }

  if (!messagesRobotAllowedToMove()) {
    running = false; // If turned off remotely
    stopMotors();
