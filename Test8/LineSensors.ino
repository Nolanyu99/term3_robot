...
bool blackLineDetectedDuringTurn()
{
  ...
  const uint16_t left_centre = target_value_at(3);
  const uint16_t middle_centre = target_value_at(4);
  const uint16_t right_centre = target_value_at(5);

  const uint8_t strong_center_count =
    (left_centre >= TURN_CENTER_DETECT_THRESHOLD ? 1 : 0) +
    (middle_centre >= TURN_CENTER_DETECT_THRESHOLD ? 1 : 0) +
    (right_centre >= TURN_CENTER_DETECT_THRESHOLD ? 1 : 0);

    const uint16_t center_sum = left_centre + middle_centre + right_centre;

    // Require at least two central sensors to see the line.
    // Also require the total central strength to be high enough.
    return strong_center_count >= 2 && center_sum >= 1500;
}
