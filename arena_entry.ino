...
constexpr uint8_t ULTRASONIC_FORWARD_TRIG_PIN = 52;
constexpr uint8_t ULTRASONIC_FORWARD_ECHO_PIN = 53;
constexpr uint8_t ULTRASONIC_SIDE_TRIG_PIN = 47;
constexpr uint8_t ULTRASONIC_SIDE_ECHO_PIN = 46;
...
enum class MissionState {
  BaseLineFollow,         // Line follow toward base exit gate
  BaseExitWait,           // Stopped at base exit gate, waiting for door
  BaseExitDriving,        // Driving forward through opened gate into tunnel
  TunnelWallFollow,       // Wall follow up the ramp
  TunnelExitWait,         // Stopped at outer door, waiting
  TunnelExitPassing,      // Driving through opened outer door
  ArenaActive,            // In the arena, main mission code runs
};
...
MissionState mission_state = MissionState::BaseLineFollow;

unsigned long calibration_start_ms = 0;
unsigned long state_start_ms = 0;
unsigned long mission_state_start_ms = 0;
...
constexpr float TUNNEL_DONE_MM           = 200.0f;
...
const char* mission_state_name()
{
  switch (mission_state) {
    case MissionState::BaseLineFollow:    return "BASE_LINE_FOLLOW";
    case MissionState::BaseExitWait:      return "BASE_EXIT_WAIT";
    case MissionState::BaseExitDriving:   return "BASE_EXIT_DRIVING";
    case MissionState::TunnelWallFollow:  return "TUNNEL_WALL_FOLLOW";
    case MissionState::TunnelExitWait:    return "TUNNEL_EXIT_WAIT";
    case MissionState::TunnelExitPassing: return "TUNNEL_EXIT_PASSING";
    case MissionState::ArenaActive:       return "ARENA_ACTIVE";
    default:                              return "unknown";
  }
}

void set_mission_state(MissionState next)
{
  if (mission_state == next) return;
  Serial.print("Mission state: ");
  Serial.print(mission_state_name());
  mission_state = next;
  mission_state_start_ms = millis();

  // Capture encoder baseline for distance-based transitions
  if (next == MissionState::BaseExitDriving) {
    base_exit_start_counts = getLeftEncoder();
  }

  Serial.print(" -> ");
  Serial.println(mission_state_name());
}
...
void drive_forward_slow()
{
  setMotors(
    LEFT_FORWARD_SIGN * (BASE_SPEED / 2),
    RIGHT_FORWARD_SIGN * (BASE_SPEED / 2)
  );
}
...
{
  float d = readUltrasonicMM(
    robot_config::ULTRASONIC_SIDE_TRIG_PIN,
    robot_config::ULTRASONIC_SIDE_ECHO_PIN
  );

  if (d > MIN_VALID_DIST_MM && d < MAX_VALID_DIST_MM) {
    lastValidSide = d;
    return d;
  }
...
{
  float d = readUltrasonicMM(
    robot_config::ULTRASONIC_FORWARD_TRIG_PIN,
    robot_config::ULTRASONIC_FORWARD_ECHO_PIN
  );
...
// =====================================================
// Encoder-based navigation primitives
// =====================================================
...
// =====================================================
// Line follow step — used in both BaseLineFollow and ArenaActive
// stops when line is lost rather than searching, so that the
// mission state machine can decide what to do next.
// =====================================================
void update_line_following_step(bool search_on_lost)
{
  ...
  if (!found) {
    ...
    if (search_on_lost) {
      ...
      // In tunnel-handoff mode: stop instead of searching
      stopMotors();
    }
...
void print_line_status()
{
  ...
  Serial.print(',');
  Serial.println(last_right_command);
}
...
void print_calibration()
{
  ...
  Serial.println("Mission starting in BASE_LINE_FOLLOW.");
}
...
// =====================================================
// Mission step — runs the appropriate behaviour for the current mission state
// =====================================================

void run_mission_step()
{
  const float fwd = readForwardDistance();
  const float side = readSideDistance();
  ...
  switch (mission_state) {
    ...
    case MissionState::BaseExitWait:
      stopMotors();
      if (fwd > BASE_DOOR_OPEN_MM) {
        set_mission_state(MissionState::BaseExitDriving);
      }
      break;

    case MissionState::BaseExitDriving:
      drive_forward_slow();
      {
          const long distance_traveled = getLeftEncoder() - base_exit_start_counts;

          if (distance_traveled < BASE_EXIT_MIN_DISTANCE_COUNTS) {
              break;
          }
      }
      ...
      if (side < SIDE_WALL_DETECTED_MM) {
          set_mission_state(MissionState::TunnelWallFollow);
      } else if (millis() - mission_state_start_ms > BASE_EXIT_DRIVE_TIMEOUT_MS) {
          Serial.println("No side wall after timeout — switching to wall follow anyway.");
          set_mission_state(MissionState::TunnelWallFollow);
      }
      break;

    case MissionState::TunnelWallFollow:
      wallFollowStep();
      if (fwd < TUNNEL_DOOR_CLOSED_MM) {
        stopMotors();
        set_mission_state(MissionState::TunnelExitWait);
      }
      break;

    case MissionState::TunnelExitWait:
      stopMotors();
      if (fwd > TUNNEL_DOOR_OPEN_MM) {
        set_mission_state(MissionState::TunnelExitPassing);
      }
      break;

    case MissionState::TunnelExitPassing:
      wallFollowStep();
      if (fwd > TUNNEL_DONE_MM) {
        set_mission_state(MissionState::ArenaActive);
      }
      break;

    case MissionState::ArenaActive:
      // Normal line follow in the arena; lost-line behaviour searches.
      update_line_following_step(true);
      break;
  }
}
...
void setup()
{
  ...
  Serial.println();
  Serial.println("=== GIGA R1 merged line-following + tunnel wall-follow + RFID + seed dispenser ===");
  ...
  pinMode(robot_config::ULTRASONIC_FORWARD_TRIG_PIN, OUTPUT);
  pinMode(robot_config::ULTRASONIC_FORWARD_ECHO_PIN, INPUT);
  pinMode(robot_config::ULTRASONIC_SIDE_TRIG_PIN, OUTPUT);
  pinMode(robot_config::ULTRASONIC_SIDE_ECHO_PIN, INPUT);
  digitalWrite(robot_config::ULTRASONIC_FORWARD_TRIG_PIN, LOW);
  digitalWrite(robot_config::ULTRASONIC_SIDE_TRIG_PIN, LOW);
  ...
  Serial.println("Then mission begins in BASE_LINE_FOLLOW.");
}
...
void loop()
{
  ...
  // RFID scanning — only active once in the arena
  if (mission_state == MissionState::ArenaActive &&
