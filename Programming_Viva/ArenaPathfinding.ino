...
constexpr ArenaPoint ARENA_ENTRANCE = {0, 7};
constexpr ArenaPoint ARENA_EXIT     = {0, 3};
...
bool arenaUpdatePoseFromRFID()
{
  ...
  char uid[32];
  uidToText(uid, sizeof(uid));

  Serial.print("arena_rfid_uid=");
  Serial.println(uid);

  // First try server localisation. The fertility reply may contain this tag's grid x/y.
  ...
  if (messagesConnected() && messagesSendIsFertile(uid)) {
    ...
    if (messagesWaitForFertilityReply(uid, 1200UL, &server_x, &server_y, &server_state)) {
  ...
  // Fallback when offline or no server position is returned.
